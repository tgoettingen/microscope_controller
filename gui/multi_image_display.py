...
class MultiImageDisplay:
    ...
    def __init__(self, parent=None):
        ...
        # Sync limits flag
        self._sync_limits = True  # Enable limits synchronization by default
        self._syncing = False  # Guard to prevent recursion during sync
    ...
    def _create_display_window(self):
        ...
        self._sync_limits_cb = self._QtWidgets.QCheckBox("Sync Limits")
        self._sync_limits_cb.setChecked(self._sync_limits)
        self._sync_limits_cb.toggled.connect(self._on_sync_limits_toggled)
        control_layout.addWidget(self._sync_limits_cb)
    ...
    def _create_plot_widgets(self):
        ...
        # Store the current range from the first plot (if any) to preserve zoom level
        existing_range = None
        ...
        old_sync = self._sync_limits
        self._sync_limits = False
        
        # Disconnect old signals before clearing widgets
        for plot_widget in self._plot_widgets.values():
            try:
                plot_widget.plotItem.vb.sigXRangeChanged.disconnect()
                plot_widget.plotItem.vb.sigYRangeChanged.disconnect()
            except:
                pass
        
        if self._overlay_plot_widget:
            try:
                self._overlay_plot_widget.plotItem.vb.sigXRangeChanged.disconnect()
                self._overlay_plot_widget.plotItem.vb.sigYRangeChanged.disconnect()
            except:
                pass
        ...
        for i, name in enumerate(self._images.keys()):
            ...
            plot_widget.setLabel('left', 'Y Position', units='')
            plot_widget.setLabel('bottom', 'X Position', units='')
            plot_widget.showGrid(x=True, y=True, alpha=0.3)
            plot_widget.setMinimumSize(300, 250)
            
            # Enable view range changes for sync
            plot_widget.plotItem.vb.sigXRangeChanged.connect(partial(self._on_range_changed_widget, plot_widget, 'x'))
            plot_widget.plotItem.vb.sigYRangeChanged.connect(partial(self._on_range_changed_widget, plot_widget, 'y'))
        ...
        # Apply coordinate ranges ONLY if we don't have an existing zoom level to preserve
        # This prevents coordinate ranges from overriding the user's zoom level
        if not existing_range:
            self._apply_coordinate_ranges()
        else:
            print(f"[MultiImageDisplay] Skipping coordinate range application to preserve zoom level")
        
        # Restore sync setting
        self._sync_limits = old_sync
        
        # If we had an existing range, apply it to all new plots to preserve zoom level
        # Do this with sync disabled to prevent triggering sync signals
        if existing_range and old_sync:
            self._sync_limits = False  # Temporarily disable sync
            try:
                x_range, y_range = existing_range
                for plot_widget in self._plot_widgets.values():
                ...
                print(f"[MultiImageDisplay] Preserved zoom range: X=[{x_range[0]:.2f}, {x_range[1]:.2f}], Y=[{y_range[0]:.2f}, {y_range[1]:.2f}]")
            finally:
                self._sync_limits = old_sync  # Restore sync
    ...
    def _create_overlay_plot(self):
        ...
        self._overlay_plot_widget.setLabel('left', 'Y Position', units='')
        self._overlay_plot_widget.setLabel('bottom', 'X Position', units='')
        ...
        # Enable view range changes for sync on overlay
        self._overlay_plot_widget.plotItem.vb.sigXRangeChanged.connect(partial(self._on_range_changed_widget, self._overlay_plot_widget, 'x'))
        self._overlay_plot_widget.plotItem.vb.sigYRangeChanged.connect(partial(self._on_range_changed_widget, self._overlay_plot_widget, 'y'))
    ...
    def _on_range_changed_widget(self, source_plot_widget, axis, view_box):
        """Handle range change from widget (using functools.partial)."""
        # Delegate to the main handler
        self._on_range_changed(source_plot_widget, axis, view_box)
    
    def _on_range_changed(self, source_plot_widget, axis, view_box):
        """Handle range change - sync all plots if sync is enabled."""
        # Guard against recursion
        if self._syncing:
            return
        
        if not self._sync_limits:
            return
        ...
        try:
            # Get the new range from the view_box
            x_range = view_box.viewRange()[0]
            y_range = view_box.viewRange()[1]
            
            # Validate ranges before applying
            if not self._is_valid_range(x_range) or not self._is_valid_range(y_range):
                print(f"[MultiImageDisplay DEBUG] Invalid range detected - skipping sync: X={x_range}, Y={y_range}")
                return
            
            # Apply to all plots except the source
            for name, plot_widget in self._plot_widgets.items():
                # Use plot widget comparison instead of ViewBox comparison
                if plot_widget != source_plot_widget:
                    try:
                        plot_widget.setXRange(x_range[0], x_range[1])
                        plot_widget.setYRange(y_range[0], y_range[1])
                    except Exception as e:
                        print(f"[MultiImageDisplay DEBUG] Error syncing to plot '{name}': {e}")
            
            # Also apply to overlay if exists and not the source
            if self._overlay_plot_widget and self._overlay_plot_widget != source_plot_widget:
                try:
                    self._overlay_plot_widget.setXRange(x_range[0], x_range[1])
                    self._overlay_plot_widget.setYRange(y_range[0], y_range[1])
                except Exception as e:
                    print(f"[MultiImageDisplay DEBUG] Error syncing to overlay: {e}")
        finally:
            # Clear guard
            self._syncing = False
    
    def _is_valid_range(self, range_tuple):
        """Check if a range tuple is valid (min < max and finite)."""
    ...
    def _on_sync_limits_toggled(self, enabled):
        """Handle sync limits checkbox toggle."""
        self._sync_limits = enabled
        print(f"[MultiImageDisplay] Sync limits {'enabled' if enabled else 'disabled'}")
    
    def sync_all_limits(self):
        """Manually synchronize X/Y limits across all displayed images."""
        ...
        try:
            # Get the range from the first plot (or focused image if set)
            source_plot = None
            if self._focused_image and self._focused_image in self._plot_widgets:
                source_plot = self._plot_widgets[self._focused_image]
            else:
                source_plot = list(self._plot_widgets.values())[0]
            
            x_range = source_plot.plotItem.vb.viewRange()[0]
            y_range = source_plot.plotItem.vb.viewRange()[1]
            
            # Validate ranges
            if not self._is_valid_range(x_range) or not self._is_valid_range(y_range):
                print(f"[MultiImageDisplay] Invalid range detected - skipping sync: X={x_range}, Y={y_range}")
                return
            
            # Apply to all plots except the source
            for name, plot_widget in self._plot_widgets.items():
                if plot_widget != source_plot:
                    try:
                        plot_widget.setXRange(x_range[0], x_range[1])
                        plot_widget.setYRange(y_range[0], y_range[1])
                    except Exception as e:
                        print(f"[MultiImageDisplay] Error syncing to plot '{name}': {e}")
            
            # Apply to overlay if exists and not the source
            if self._overlay_plot_widget and self._overlay_plot_widget != source_plot:
                try:
                    self._overlay_plot_widget.setXRange(x_range[0], x_range[1])
                    self._overlay_plot_widget.setYRange(y_range[0], y_range[1])
                except Exception as e:
                    print(f"[MultiImageDisplay] Error syncing to overlay: {e}")
            
            print(f"[MultiImageDisplay] Synced all limits to X: [{x_range[0]:.3f}, {x_range[1]:.3f}], Y: [{y_range[0]:.3f}, {y_range[1]:.3f}]")
        finally:
            self._syncing = False
    
    def _toggle_sync_limits(self):
        """Toggle sync limits from context menu."""
        self._sync_limits = not self._sync_limits
        if self._sync_limits_cb:
            self._sync_limits_cb.setChecked(self._sync_limits)
        print(f"[MultiImageDisplay] Sync limits {'enabled' if self._sync_limits else 'disabled'}")
    ...
    def _show_context_menu(self, pos, name: str):
        ...
        # Sync limits option
        sync_action = menu.addAction("Sync Limits")
        sync_action.setCheckable(True)
        sync_action.setChecked(self._sync_limits)
        sync_action.triggered.connect(lambda: self._toggle_sync_limits())
    ...
    def get_current_ranges(self):
        """Get current view ranges for all plots (for debugging)."""
        ranges = {}
        for name, plot_widget in self._plot_widgets.items():
            ranges[name] = plot_widget.plotItem.vb.viewRange()
        if self._overlay_plot_widget:
            ranges['overlay'] = self._overlay_plot_widget.plotItem.vb.viewRange()
        return ranges
